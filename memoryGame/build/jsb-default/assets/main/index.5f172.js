  ...
  cards: [ function(require, module, exports) {
    ...
    var Card = function(_super) {
      ...
      Card.prototype.unreveal = function() {
        if (!this._isOpen || this.animationNode.getNumberOfRunningActions() > 0) return;
        var callFunc1 = cc.callFunc(function() {
    ...
    var GamePlay = function(_super) {
      ...
      GamePlay.prototype.onDisable = function() {
        if (this.node.parent.getComponent("home").isforSetting) SoundManager_1.default.getInstance().stopAllSounds(); else {
          ...
          this.levelLabel.node.parent.scale = .5;
      ...
      GamePlay.prototype.updateGamePlay = function() {
        ...
        this.levelLabel.node.parent.runAction(cc.sequence(cc.scaleTo(1.5, 1), cc.delayTime(.5), cc.callFunc(function() {
          ...
          (_this._level + 1) % 10 != 1 || JSON.parse(cc.sys.localStorage.getItem("shownTutorial" + _this._level)) ? _this.startGame() : _this.showTutorials();
        })));
      ...
      GamePlay.prototype.flipAllCards = function() {
        for (var _i = 0, _a = this.gameLayout.node.children; _i < _a.length; _i++) {
          ...
          ((this._level + 1) % 10 != 1 || JSON.parse(cc.sys.localStorage.getItem("shownTutorial" + this._level))) && child.getComponent("cards").disableOverlay();
        }
        if ((this._level + 1) % 10 != 1 || JSON.parse(cc.sys.localStorage.getItem("shownTutorial" + this._level))) this.isTutoiral = false; else {
          var demoCard_1 = this.levelData.demoCard;
          this.tutorialCards = this.gameLayout.node.children.filter(function(item) {
            return item.getComponent("cards").getCardName() == demoCard_1;
          });
          this.isTutoiral = true;
          this.moveTutHand();
        }
      ...
      GamePlay.prototype.showCard = function(card) {
        var _this = this;
        this.cardsInPair.forEach(function(card) {
          console.log("card in show card", card.getComponent("cards").getCardName(), _this.cardsInPair.length, _this.isTouchBlocked);
        });
        0 == this.cardsInPair.length && 0 == this.mirrorOPenCards.length && (this.currentCardName = card.getComponent("cards").getCardName());
        if (this.cardsInPair.length >= this.groupOf && 0 == this.mirrorOPenCards.length || this.isTouchBlocked) return;
      ...
      GamePlay.prototype.addToOpenList = function() {
        ...
        if (isMatch) {
          ...
          if (this.levelData.gameType == constants_1.GAME_TYPE.MIRROR) {
            ...
            cards.length = 0;
            for (var i = 0; i < this.mirrorOPenCards.length; i++) cards.push(this.mirrorOPenCards[i].getComponent("cards"));
          }
          ...
          this.node.runAction(cc.sequence(cc.delayTime(.3), cc.callFunc(function() {
            ...
            _this.mirrorOPenCards.length = 0;
          })));
          for (var _i = 0, cards_1 = cards; _i < cards_1.length; _i++) {
            var cardScript = cards_1[_i];
      ...
      GamePlay.prototype.isMirrorCardPresent = function() {
        var currentCard = this.currentCardName;
        var lastCharacter = currentCard[currentCard.length - 1];
        var nextCard = currentCard.replace(lastCharacter, "1" == lastCharacter ? "2" : "1");
        var count = 0;
        this.mirrorOPenCards = __spreadArrays(this.mirrorOPenCards, this.cardsInPair);
        this.cardsInPair.length = 0;
        for (var _i = 0, _a = this.gameLayout.node.children; _i < _a.length; _i++) {
          var child = _a[_i];
          child.getComponent("cards").getCardName() != nextCard || child.getComponent("cards").isOpen() || count++;
        }
        this.currentCardName = nextCard;
        console.log("next card is", nextCard, count);
        return 0 != count;
      };
      GamePlay.prototype.isPair = function(cards) {
      ...
      GamePlay.prototype.continueTutorial = function() {
        ...
        0 == this._level ? this.gameTutorials.getComponent("tutorials").moveToNextSlide() : this.gameTutorials.getComponent("tutorials").showTerminationPopUp();
      };
      ...
      GamePlay.prototype.stopTutorials = function() {
        ...
        var openCards = this.gameLayout.node.children.filter(function(item) {
        ...
        openCards.forEach(function(card) {
          return card.getComponent("cards").unreveal();
        });
        this.OpenCards.length = 0;
        console.log("open cards here", this.OpenCards);
        cc.sys.localStorage.setItem("shownTutorial" + this._level, true);
        for (var _i = 0, _a = this.gameLayout.node.children; _i < _a.length; _i++) {
          var child = _a[_i];
      ...
      GamePlay.prototype.openPairCards = function() {
        ...
        if (0 != this.cardsInPair.length && this.cardsInPair.length <= this.groupOf) hiddenCards = this.cardsInPair; else {
          console.log("inside thiswe are here");
          if (0 == this.mirrorOPenCards.length) {
            hiddenCards = this.gameLayout.node.children.filter(function(item) {
              return !item.getComponent("cards").isOpen();
            });
            this.currentCardName = hiddenCards[0].getComponent("cards").getCardName();
          } else hiddenCards = this.gameLayout.node.children.filter(function(item) {
            return item.getComponent("cards").getCardName() == _this.currentCardName;
          });
          console.log("hidden cards", hiddenCards);
        }
        if (hiddenCards.length > 0) {
          var card_1 = this.currentCardName;
          var pairs = this.gameLayout.node.children.filter(function(item) {
            return item.getComponent("cards").getCardName() == card_1;
          });
      ...
      GamePlay.prototype.checkIsTutPoint = function() {
        if (0 == this._level) return;
        console.log("current open card length is ", this.cardsInPair.length, this.levelData.groupOf);
        switch (this._level) {
         case 10:
          this.cardsInPair.length == this.levelData.groupOf - 1 && this.gameTutorials.getComponent("tutorials").showGameInfoPopUp();
          break;

         case 20:
          console.log("inisde this but not sure if this will work");
          this.cardsInPair.length == this.levelData.groupOf && 0 == this.mirrorOPenCards.length && this.gameTutorials.getComponent("tutorials").showGameInfoPopUp();
          break;
    ...
    var Home = function(_super) {
      ...
      Home.prototype.setupUI = function() {
        ...
        if (!this.terms.active) {
          ...
          rewardGivenAt != today ? this.dailyRewards.active = true : this.startGame();
        }
      };
      ...
      Home.prototype.updateLevelSelctionScreen = function() {
        var levels = GameManager_1.GameManager.getInstance().getLevelInfo();
        var startIndex = 0;
        var totalFrame = Math.ceil(levels.length / 10);
        for (var _i = 0, _a = this.scrollViewLayout.node.children; _i < _a.length; _i++) {
          var child = _a[_i];
      ...
      Home.prototype.onRewardCollected = function() {
        this.isAfterTerms || this.showLevelSelection();
      };
  ...
  levelUIManager: [ function(require, module, exports) {
    ...
    var LevelUIManager = function(_super) {
      ...
      LevelUIManager.prototype.updateLevels = function(startIndex) {
        ...
        var levelsInfo = JSON.parse(cc.sys.localStorage.getItem("LevelInfo"));
        var levelsInfoForMode = JSON.parse(levelsInfo.level);
        for (var _i = 0, _a = this.layout.node.children; _i < _a.length; _i++) {
          var button = _a[_i];
  ...
  tutorials: [ function(require, module, exports) {
    ...
    var Tutorial = function(_super) {
      ...
      function Tutorial() {
        ...
        _this.slideIndex = 0;
        _this.delegate = null;
        _this.bgForTut3 = null;
        _this.bgForTut4 = null;
        _this.title = null;
        return _this;
      }
      Tutorial.prototype.start = function() {};
      Tutorial.prototype.showMemoriseTime = function(time) {
        var keyString = this.getGameName();
        ...
        this.title.string = GameManager_1.GameManager.getInstance().getString("title" + keyString);
      };
      Tutorial.prototype.onAccept = function() {
        SoundManager_1.default.getInstance().playEffect(this.audioClip, false);
        0 == this.delegate._level || 0 == this.slideIndex ? this.moveToNextSlide() : this.node.active = false;
      };
      Tutorial.prototype.terminateTutorials = function() {
        this.slideIndex = 4;
      ...
      Tutorial.prototype.updateUI = function(bgTexture, activeButton) {
      ...
      Tutorial.prototype.updateTutorialText = function(text) {
      ...
      Tutorial.prototype.showGameInfoPopUp = function() {
        this.node.active = true;
        this.updateUI(this.bgForTut3, 3);
        var key = "info" + this.getGameName();
        console.log("key is ", key);
        var text = GameManager_1.GameManager.getInstance().getString(key);
        console.log("text is ", text);
        this.updateTutorialText(text);
        this.content.fontSize = 64;
        this.title.fontSize = 70;
      };
      Tutorial.prototype.showTerminationPopUp = function() {
        this.updateUI(this.bgForTut3, 2);
        var text = GameManager_1.GameManager.getInstance().getString("endTutorials");
        console.log("text is ", text);
        this.updateTutorialText(text);
      };
      Tutorial.prototype.getGameName = function() {
        var keyString = "";
      ...
      __decorate([ property(cc.SpriteFrame) ], Tutorial.prototype, "bgForTut3", void 0);
      __decorate([ property(cc.SpriteFrame) ], Tutorial.prototype, "bgForTut4", void 0);
